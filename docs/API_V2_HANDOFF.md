...
| Interview | Scenario/Session、短期 realtime connection、transcript、end/report Job、证据与 rubric 校验；无录音的 session 可完整结束并生成报告。 |
...
当前 Alembic 单一 head 是 `20260723_0027`，从既有 `20260722_0012` 线性演进：
...
1. **Interview 音视频录制尚未接入真实 capture/finalizer/storage provider。** 当前能力显式为
   `audio_recording=false`、`video_recording=false`；请求录音或录像会在 Session 持久化前返回
   `interview.recording_unavailable`。无录音、保留 transcript 的流程可用。
...
3. **真实外部能力必须由部署完成。** Connection provider allowlist、S3-compatible object
   store、ClamAV、SMTP、模型/embedding、realtime signaling/TURN 和 XeLaTeX sandbox 都没有
