...
retention 和 model route。Realtime endpoint 每次只签发短期、单 session、单 audience HMAC
credential；持久 Session 而非连接状态是权威事实。

End/report worker 从 persisted Job kind 分派。无录音 session 的 end 不依赖媒体 provider；report
provider 输出严格 JSON，随后验证 rubric dimension、score scale、权重、transcript segment 与
evidence time range，才发布 Report。开发模式可使用明确标注的确定性 report adapter；部署环境
必须使用真实模型，且不会把全局 fallback 配置冒充用户对跨 provider 数据处理的同意。

当前 composition 没有音视频 capture/finalizer/storage provider，因此录音/录像 capability 为
false；请求对应 consent 会在写 Session 前稳定失败。Transcript-only 是已实现路径。
...
`20260722_0008` 至 `20260723_0027` 是单 head 线性链。详细 revision 作用见
...
- Interview 音视频录制 provider 未接入；仅 transcript/no-recording 路径可用。
...
```bash
uv sync --locked
uv run alembic heads                     # 必须只有 20260723_0027
uv run ruff check .
uv run mypy --strict
uv run pytest -q
git diff --check
