...
## 2026-07-22 前后端 HTTP 对齐增量说明

当前已补充当前身份与 workspace 只读投影、Interview 场景目录与 Session 列表、Knowledge 强 ETag 条件更新，以及生产 OpenAI-compatible embedding adapter。`/me` 和 workspace 路径只读取已经由身份中间件验证的主体与成员关系，**不**提供密码、注册、登录、OIDC 或 token 签发能力。新增路径在共享仓库冻结 method/path 契约前均保留 `x-contract-status: mock`，前端可以联调，但不能将当前请求/分页外形视为已发布协议。
...
`workspace-shared-docs/contracts/v1/ai-job-workspace.contract.schema.json` 是唯一正式的机器可读数据结构来源；`workspace-shared-docs` 是固定到明确 commit 的 submodule。它不是 OpenAPI 或 AsyncAPI：目前没有完整、可发布的 `method + path + request + response + header + status` 契约包；部分已实现路由仅通过 FastAPI 暴露 OpenAPI 绑定。本文件不修改、更不推断正式契约；它记录当前实现的覆盖面和所有明确的 mock 边界。
...
| 领域 | Path | 状态 | 缺口与处置 |
|---|---|---|---|
| 身份、Workspace、审计 | 可信代理身份断言；`/me`、workspace/member/invitation/audit/SSE 路径 | 身份边界已实现；当前用户、workspace 与成员只读路径为 Mock | HTTP/WS 可验证 `trusted_proxy_hmac` v1 断言；`/me`、workspace 列表/详情/成员列表会检查当前 scope 与成员资格。登录、注册、凭据/token、成员变更、邀请与审计 SSE 仍未实现。 |
...
| Resume proposal | proposal create/list/GET/decision | 响应与 decision 为正式结构；create 为 Mock | `ResumeProposal` 与 `ProposalDecisionRequest` 已校验，支持按 Resume 恢复 pending Proposal；非 mock 模式已调用配置的文本模型，并在发送证据前逐来源执行外部处理授权，Proposal 仍须显式接受才会写入 Resume。create 请求仍是临时 mock adapter。 |
...
| Knowledge 生命周期 | PATCH/DELETE/version/sync/SSE/access evaluation | visibility PATCH 为 Mock；其余未实现 | GET 返回强 ETag；PATCH 强制 `If-Match` 并用 repository CAS 防止多 worker 丢失更新。DELETE、同步、SSE 与公开授权评估协议仍未实现。 |
| 上传与外部连接 | upload、connection、authorization session | 未实现 | 即便已有资源定义，multipart complete、一次性 token 和 secret 生命周期仍必须显式定义。 |
| Interview scenario | scenario list/create/detail/PATCH | 内置 list/detail 为 Mock；create/PATCH 未实现 | 当前返回稳定、Schema 校验的内置场景目录；持久化场景、创建/更新 body、ETag 与正式分页协议仍未冻结。 |
...
| Interview 其他路径 | session list、report job、transcript、SSE | session list 为 Mock；其余未实现 | Session 列表支持 scope、status 与不透明 cursor；需另行定义报告再生成、transcript 分页/artifact 与 job/event 流。 |
...
该桥仍属于 v0.1 的内部索引实现：它从 SIR 的用户可读 title/profile/sections 提取有界文本。mock 配置使用确定性 embedding；非 mock 配置使用 OpenAI-compatible `/embeddings` adapter，并严格校验 provider/model/revision/dimension/metric/normalization，禁止把旧空间与新空间混写。模型或维度变化需要受控重建索引。`pinned` 会话语义、Resume 删除时的 source stale/deleted 生命周期、索引 Job SSE 和跨 worker/outbox 重试仍未冻结为公开契约。
...
- **ETag**：Resume GET、artifact content 与 KnowledgeSource GET 有 ETag；Knowledge visibility PATCH 强制 `If-Match`。其它资源尚未统一条件请求和更新并发语义。
...
2. **HMAC 不是登录 API。** backend 只验证可信代理断言；它不验证密码或 Bearer/OIDC credential，不签发或撤销 token。当前 `/me`、workspace 与 member 只读投影会查询已有身份/成员记录，但不是登录、注册、邀请或成员管理产品契约。上游认证代理仍必须先认证主体，再为最终原始 request target 重签名。
