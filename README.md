...
v0.1.0 是一个面向 AI 求职工作流的模块化单体（modular monolith）后端。产品面严格实现 API STANDARD V2：Hosted Identity 与 OAuth 2.0/OIDC、Workspace/Resume、Knowledge、Agent、Interview、统一 Job/Artifact/SSE/Audit，以及 PostgreSQL/pgvector 持久化；另提供只读运维 Dashboard。
...
包含 bootstrap、migration、遥测清理、交互式 shell 与容器启动五个用例，以及与唯一消费者共置的
窄端口和强类型进度契约，`infrastructure` 实现 JSONC、PostgreSQL、Alembic 与本地进程适配器，
`interfaces` 只负责 CLI/容器输入输出；
`composition.py` 是按命令只装配所需能力的唯一组合根。核心层的依赖方向由 AST 测试执行，
而不是只靠目录约定。分层取舍见
[ADR 0004](docs/decisions/0004-dbctl-layered-domain-architecture.md)，操作者输出与安全诊断见
[ADR 0005](docs/decisions/0005-dbctl-operator-diagnostics.md)。

独立的 `workspace-shared-docs` submodule 是前后端共享契约的唯一事实来源；API V2 运行时只读取 `contracts/v2/schema.jsonc`，该文件会原样打入 backend wheel。实现状态与交接证据见 [API V2 implementation](docs/API_V2_IMPLEMENTATION.md) 和 [handoff](docs/API_V2_HANDOFF.md)。V1 schema 仅为显式并行迁移测试保留，不是 V2 回退源；`api.legacy_v1_enabled` 默认关闭且在 staging/production 强制为 `false`。首次 clone 必须使用 `git clone --recurse-submodules`，已有 checkout 则执行 `git submodule update --init --recursive`。
...
本地默认值使用确定性的内存 repository、mock 模型，以及真实的 Hosted Identity → OAuth PKCE → V2 Bearer 边界；它们不需要 PostgreSQL 或外部模型密钥，也绝不能用于 staging/production。`X-Mock-*` 只属于 development/test 中显式开启的 V1 并行迁移面，默认 V2 不读取这些头。memory 模式没有伪造跨领域统一投影：任何需要 durable worker，或读取 Job/Artifact/Event/Audit 统一投影的产品请求都会明确返回可重试 `503 service.durable_runtime_required`；完整 V2 纵向流程必须使用 PostgreSQL。
`backend` 默认读取当前工作目录的 `config.jsonc`，也可用 `--config` 或 `AIWS_CONFIG` 覆盖；日志与知识库 blob 的相对路径统一以该配置文件所在目录为基准，因此 wheel 安装态不会把运行数据误写进虚拟环境。

### Agent 检索增强生成

当 `AgentRunRequest.knowledge.mode` 不是 `none` 时，V2 worker 在模型调用前重新计算 execution
grant，并在结果提交前再次计算和逐值比较。检索发生在数据库事务外，但查询携带真实 actor、
Workspace 以及精确的 source/version/policy allowlist；同一 embedding space 中的向量与词法
混合检索最多返回 20 个稳定编号候选。检索文本始终是不可信证据，不能覆盖系统指令；授权、
会话 revision 或 Resume base revision 在模型执行期间变化时，模型结果会被丢弃并以稳定问题
终结 Run。

模型请求使用 provider-native strict JSON Schema，且只有 provider capability discovery 明确
支持结构化输出后才发送网络请求。模型只能选择服务端候选编号；正式 citation provenance 由
服务端物化，并与 assistant message、Run/Job 终态和 outbox 原子提交。若请求 Resume 修改，
模型只返回无身份的 operation 草案；服务端按闭合 operation union 解析、生成稳定 ID 和预览，
再在同一事务中创建可审阅 Proposal，绝不直接写 Resume。生产组合根显式安装空 tool registry，
未注册工具稳定失败为 `agent.tool_unavailable`，不会制造永远无法执行的 approval。
...
Resume 的同步修改在一个短事务中写入聚合、不可变 revision、操作批次 receipt 与通知事件。Import、restore、render 使用统一 Job + transactional outbox：提交工作意图后，独立 dispatcher 以租约领取，worker 在事务外解析或渲染，再以第二个短事务原子发布 Job 终态、Artifact/content/source-map 和通知。稳定 operation ID、CAS（compare-and-swap）与耗尽补偿保证重放不会制造重复结果或永久 `queued`。

PDF 编译、非可信文档解析和 embedding 计算不持有数据库事务。Resume import 与 Knowledge
上传的 TXT/Markdown/PDF/DOCX parser，以及 XeLaTeX renderer，都在独立 session/进程组中执行；
生产模式在加载第三方 parser/compiler 前施加 Landlock 文件系统 allowlist、libseccomp syscall
denylist、CPU/地址空间/文件/FD 等 rlimit、输入/结果大小上限与墙钟 deadline，超时后回收整个
进程组。Knowledge parser 还把结构化 part 数与索引上限对齐，并在父进程按闭合 JSON 协议重建
locator，避免小输入放大成无界结果。Resume source map 在写 ORM
前先构造成领域对象，并与实际 PDF 页数、Artifact identity、Resume immutable revision 及
canonical entity/field path 交叉验证；失败时整个结果事务回滚，Job 进入脱敏终态。Knowledge
重建索引时，当前 `ready` 版本持续可检索，只有新版本完整计算成功才切换；失败时继续保留旧
版本。对象 blob 位于数据库外，使用稳定对象键与补偿删除，不能宣称为跨对象存储和 PostgreSQL
的 ACID 事务。
...
1. 将部署配置设为 `"environment": "production"`、`"database.mode": "postgresql"` 与 `"api.legacy_v1_enabled": false`。生产/预发布环境若仍使用任何 mock 产品能力，后端会拒绝加载配置。
2. 后端仅监听私有 loopback、Unix socket 或受限 Pod 网络；安全组、容器网络和防火墙必须禁止客户端绕过入口认证代理直连它。
3. Hosted Identity 完成用户认证；OAuth Authorization Code + PKCE 签发短期 Bearer token。每个 V2 用例仍在数据库事务中依据 token scope、真实用户、membership/role 与路径 Workspace 重新授权，Nginx 不签发业务身份。
...
6. staging/production 的 Linux 宿主必须为 Resume import、Knowledge 文件 parser，以及启用 `resume_rendering.adapter: xelatex` 时的 renderer，提供可工作的 Landlock（ABI ≥ 3）与 seccomp；Bubblewrap 可用时会叠加使用。无法建立强隔离时实现会 fail closed，而不是退化为不受限子进程。
...
|---|---|---|
| `AIWS_CURSOR_HMAC_SECRET` | 全部 backend worker | 至少 32 bytes；所有 worker 共享，且不得复用 sensitive-idempotency 等其他密钥域 |
...
`dbctl` 将可管道化的主结果写入 stdout，将命令意图、逐阶段进度、警告和错误诊断写入
stderr。命令会在配置或数据库 I/O 前声明子命令、模式与配置来源；进度使用稳定逐行文本，不用
spinner 或光标回写。`--quiet` 只抑制声明和正常进度，主结果与失败仍会输出；`--no-color`、存在
即生效的 `NO_COLOR` 或 `TERM=dumb` 会禁用 ANSI 颜色。通用选项既可写在子命令前，也可自然地
写在子命令后，例如：

```bash
# 只保留 stdout 主结果；失败诊断仍在 stderr。
uv run dbctl migrate --revision head --quiet

# 为持久日志或辅助技术输出稳定纯文本。
NO_COLOR=1 uv run dbctl prune-telemetry --apply --quiet
```

异常失败会给出短原因、可执行建议和安全 traceback（回溯）。渲染器不构造
`TracebackException`，也不读取 locals、源码行、任意 driver/subprocess 异常正文、动态异常类名
或 Python `__notes__`。只有 frame 的 globals 字典与 `sys.modules` 中真实 dbctl 模块 namespace
为同一对象，且模块文件与 code filename 匹配时，才显示通过白名单的文件/函数元数据；仅伪造
package 路径无法获得信任。其他栈帧隐藏文件与函数，只保留固定占位和行号。内部错误正文还必须
来自精确的 dbctl 错误类型、可信最终 raise frame 和构造时冻结的正文快照；展示阶段绝不重新读取
可变的 `BaseException.args`。第三方子类不能借内部基类放行正文。除 dbctl 定义的安全动作标签外，
通用外部错误代理只公开固定的内建异常类别，并仅对 `OSError` 通过内建 descriptor 安全读取
有界整数 `errno`；不会反射读取 `SQLSTATE`、
行列号或其他第三方属性。只有 dbctl 构造、再次脱敏，并由进程内模块密钥 HMAC 绑定异常身份与
全部字段的不可变安全正文/注释可以显示；读取载荷后替换正文、notes、来源授权位或把载荷移植到
另一个异常都会 fail closed。这个完整性标签用于抵御捕获后的异常对象篡改，但不把同一解释器
变成隔离边界：已经能读取 dbctl 模块私有密钥、执行任意 Python 代码的一方也能调用诊断 helper
或直接写 stderr，必须依赖依赖项审阅、进程隔离和操作系统权限来防御。不要在故障工单中另行
粘贴 DSN、密码或私密配置；需要底层数据库细节时，应查询权限受控的 PostgreSQL server log。

`bootstrap --dry-run` 的 stdout 保持既有字节序列，只包含原有的脱敏 SQL 计划；命令意图、配置
初始化与 target/stage 进度等新增信息只写 stderr，因而不会破坏消费原输出的脚本。

进度按真实提交边界陈述运维影响：bootstrap 失败会报告此前完成的阶段；若操作者在阶段调用中
中断，则保守地把当前阶段标为“完成状态未知”，并在退出码 `130` 的取消摘要中保留安全影响说明。
遥测清理失败会报告此前已经提交的短事务和删除数，且同一轮始终使用固定 cutoff；migration
失败时应先核验当前
revision 再重试。stderr 关闭或输出 sink 故障只会损失展示，不会回滚已提交状态、触发额外动作
或改变命令退出语义。退出码保持为：成功 `0`，可展示的参数/配置/运行时失败 `2`，未预期内部
失败 `1`，操作者取消 `130`；`shell` 返回规范化后的 `psql` 状态。容器启动也是独立应用用例：
缺少目标命令或预期的配置投影失败返回 `2`，目标命令无法 `exec` 或发生未预期内部失败返回
`1`；`exec` 成功后入口进程被目标程序替换，后续退出状态由目标程序自身决定。
...
旧浏览器诊断入口 `POST /api/v1/diagnostics` 只在 development/test 显式开启
`api.legacy_v1_enabled` 时存在；它不属于 API STANDARD V2，也不在生产公网面。该入口只接受经过
旧身份边界的严格 error/performance/network
...
产品 API 使用 Hosted Identity + OAuth 2.0/OIDC。公开 discovery/JWKS、Authorization Code + PKCE、
refresh rotation/reuse detection、revocation、`/userinfo` 与 Hosted Identity 流均由 backend 提供；
Resource Server 只接受 `Authorization: Bearer`，并验证 issuer、audience、签名、时效、token family、
session/user 状态与 scope。路径中的 Workspace 从不由客户端 header 决定；应用服务在每次读写中
重新加载真实 principal 与 membership，并以 action-specific scope/role policy 授权。
...
以下协议只服务于 development/test 中显式开启的 V1 并行迁移，不是 V2 身份、授权或回退路径。
旧入口认证代理应在验证用户凭证、选择 workspace 并执行成员资格/角色授权后，为最终转发到
backend 的原始请求目标签发这些 header：
...
生产配置拒绝开启 V1，公网 Nginx 对 `/api/v1/` 固定返回 404。不要把该旧 HMAC 协议接到 V2，
也不要企图用客户端提供的 assertion header 绕过 OAuth 与数据库授权。
...
- Workspace SSE 必须经 TLS Nginx；该精确路径关闭 proxy buffering/gzip，保留长读超时和 `Last-Event-ID`。Interview realtime 返回短期、绑定 session/audience 的服务端凭据，不把 WebSocket 媒体流伪装成 backend HTTP 路由。
- 生产禁用 FastAPI docs；示例公网 Nginx 只代理 discovery、OAuth、Hosted Identity、`/userinfo` 与 `/api/v2/`，不转发 V1、`/docs`、`/openapi.json`、Dashboard 或内部 health 路径。
...
发布前请把这些限制与 [API V2 交接清单](docs/API_V2_HANDOFF.md) 一起纳入威胁建模、反向代理集成测试和客户端兼容性测试。历史 `CONTRACT_GAPS.md` 已归档，不能再作为当前协议或实现状态依据。
