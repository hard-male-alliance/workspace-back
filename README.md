...
| `workspace-dbctl` | 显式 bootstrap、Alembic migration、遥测保留期清理与 `psql` shell | bootstrap 使用本机 sudo；其余使用迁移器/目标身份；后端启动永不自动迁移 |
...
以下 DSN 必须是相互独立的凭证，不能以 application DSN 代替 dashboard 或 migrator DSN。生成的角色密码只写入权限为 `0600`、被 Git 忽略的本地 `config.jsonc`；DSN 和 HMAC secret 仍不得写入可提交文件、日志、systemd unit 命令行或 shell history。bootstrap 不接受管理员 DSN，固定通过终端验证的本机 `sudo -u postgres` 执行。
...
以下命令均假设已经审阅 `dbinit.jsonc`，且当前终端账户可以通过 sudo 验证切换为其中配置的本机 PostgreSQL 账户。若 `config.jsonc` 不存在，第一次 dbctl 调用会复制同目录 `example.jsonc`、生成三个登录角色密码并将文件权限收紧为 `0600`。后端不会在启动时创建 schema、创建角色或运行 Alembic。

```bash
# 1. 先审阅脱敏、无连接的 bootstrap 计划
uv run workspace-dbctl --config config.jsonc bootstrap --dry-run

# 2. 固定通过 sudo -u postgres 执行幂等 role/database/schema/权限计划；sudo 会走终端验证
