...
class RecordingTelemetryRunner:
    ...
    @param stale_remaining 最后统计返回的过期记录数 / Stale-record count returned by final count.
    @param delete_calls 已记录的删除调用 / Recorded deletion calls.
    @param count_calls 已记录的计数调用 / Recorded count calls.
    """

    batch_results: list[int]
    stale_remaining: int
    delete_calls: list[tuple[datetime, int, int]] = field(default_factory=list)
    count_calls: list[tuple[datetime, int]] = field(default_factory=list)
    ...
    def delete_batch(
        ...
        *,
        ...
        """
        self.delete_calls.append((cutoff, batch_size, statement_timeout_ms))
        return self.batch_results.pop(0) if self.batch_results else 0

    def remaining_count(self, *, cutoff: datetime, statement_timeout_ms: int) -> int:
        """@brief 记录并返回预设的剩余数量 / Record and return the configured remaining count.

        @param cutoff 过期边界 / Staleness boundary.
        @param statement_timeout_ms 语句超时 / Statement timeout.
        @return 预设的剩余过期数量 / Configured number of stale rows remaining.
        """
        self.count_calls.append((cutoff, statement_timeout_ms))
        return self.stale_remaining
...
class RecordingConnection:
    ...
    @param stale_remaining COUNT 查询应返回的数值 / Value returned by COUNT query.
    ...
    def execute(
        ...
        if "RETURNING telemetry.id" in sql:
            return FakeCursor([("telemetry",)] * self.deleted_rows)
        if "count(*)" in sql:
            return FakeCursor([(self.stale_remaining,)])
        return FakeCursor([])
...
def test_prune_dry_run_is_default_and_never_calls_runner() -> None:
    ...
    runner = RecordingTelemetryRunner(batch_results=[1], stale_remaining=99)
...
def test_apply_uses_bounded_batches_and_reports_remaining_count() -> None:
    """@brief apply 仅按上限分批删除并在末尾报告精确剩余数 / Apply must delete only bounded batches and report exact remaining count."""

    runner = RecordingTelemetryRunner(batch_results=[3, 3, 1], stale_remaining=17)
    result = TelemetryPruneExecutor(runner, clock=_fixed_now).execute(
        TelemetryPruneRequest(
            retention_days=7,
            batch_size=3,
    ...
    assert result.remaining_count == 17
    assert result.reached_batch_limit is False
    assert all(call[1:] == (3, 1_234) for call in runner.delete_calls)
    assert runner.count_calls == [(datetime(2026, 7, 8, 12, 0, tzinfo=UTC), 1_234)]
    assert "删除 7 条，仍有 17 条" in result.render_operator_summary()


def test_apply_stops_at_configured_batch_budget() -> None:
    """@brief max_batches 必须成为硬边界 / max_batches must be a hard boundary."""

    runner = RecordingTelemetryRunner(batch_results=[2, 2, 2], stale_remaining=6)
    result = TelemetryPruneExecutor(runner, clock=_fixed_now).execute(
        TelemetryPruneRequest(
            retention_days=1,
            batch_size=2,
...
def test_psycopg_runner_sets_local_owner_and_binds_values(
    ...
    deleted = runner.delete_batch(cutoff=cutoff, batch_size=4, statement_timeout_ms=1_234)
    ...
    delete_sql, delete_parameters = connection.calls[2]
...
def test_cli_apply_is_the_only_deletion_opt_in(
    ...
    observed_arguments: list[tuple[bool, int, int, int]] = []
    ...
    def fake_prune(
        ...
        *,
        ...
        """
        observed_arguments.append((apply, batch_size, max_batches, statement_timeout_ms))
        return TelemetryPruneResult(
            ...
            remaining_count=3 if apply else None,
        )
    ...
    assert exit_code == 0
    assert observed_arguments == [(True, 7, 2, 2_500)]
    assert "删除 2 条，仍有 3 条" in captured.out
