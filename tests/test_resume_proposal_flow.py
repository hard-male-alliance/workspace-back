...
class _ProposalProvider:
    ...
    async def stream_text(
        ...
        assert request["inference"]["allow_external_model_processing"] is True
        yield "Experienced with Python async APIs and FastAPI."
...
def test_production_proposal_uses_model_after_source_policy_authorization(
    backend_client: TestClient,
) -> None:
    """A non-mock proposal is generated by the model and remains reviewable."""
    visibility = _proposal_visibility()
    visibility.update(
        {
    ...
    source_response = backend_client.post(
        "/api/v1/knowledge-sources",
        json={
            "name": "Authorized proposal evidence",
            "source_type": "manual_note",
            "content": "Built Python async APIs with FastAPI.",
            "visibility": visibility,
        },
        headers=idempotency_headers("proposal-model-evidence-0001"),
    )
    assert source_response.status_code == 201, source_response.text
    source = source_response.json()
    ingestion_response = backend_client.post(
        f"/api/v1/knowledge-sources/{source['id']}/ingestion-jobs",
        headers=idempotency_headers("proposal-model-ingest-0001"),
    )
    job = wait_for_json(
        backend_client,
        f"/api/v1/knowledge-ingestion-jobs/{ingestion_response.json()['id']}",
        lambda payload: payload["status"] in {"succeeded", "failed"},
    )
    assert job["status"] == "succeeded", job

    app = cast(Any, backend_client.app)
    container = cast(BackendContainer, app.state.container)
    provider = _ProposalProvider()
    container.proposals._provider = provider
    container.proposals._ai = replace(
        container.settings.ai,
        provider="openrouter",
        data_region="global",
    )
    resume = _create_resume(backend_client, "proposal-model-resume-0001")
    response = backend_client.post(
        f"/api/v1/resumes/{resume['id']}/proposals",
        json={
            "instruction": "Write a concise backend summary",
            "source_ids": [source["id"]],
            "render_hint": "preview",
        },
        headers=idempotency_headers("proposal-model-create-0001"),
    )
    assert response.status_code == 201, response.text
    proposal = response.json()
    value = proposal["operations"][0]["value"]
    assert value["plain_text"] == "Experienced with Python async APIs and FastAPI."
    assert provider.prompts and "Built Python async APIs with FastAPI" in provider.prompts[0]
