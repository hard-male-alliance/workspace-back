...
class V2InterviewHttpAdapter:
    ...
    async def create_connection(
        ...
        async def operation() -> ReplayableResponse:
            ...
            _validate_realtime_connection_response(runtime.contracts_v2, payload)
            return _private_replayable(
                payload,
                201,
...
def _validate_realtime_connection_response(
    validator: ContractDefinitionValidator,
    payload: dict[str, JsonValue],
) -> None:
    """@brief 将服务端 RealtimeConnection 契约错误分类为服务故障 / Classify a server-authored RealtimeConnection contract error as a service fault.

    @param validator API v2 共享契约 validator / API v2 shared-contract validator.
    @param payload 服务端刚生成且不写日志的敏感连接表示 / Sensitive connection
        representation just generated by the server and never logged.
    @return 无返回值 / No return value.
    @raise DomainError 响应不符合共享契约时以稳定 500 Problem 失败 / Raised with a stable
        500 Problem when the response violates the shared contract.
    @note 不能把服务端生成错误误报成用户提交 422；错误响应不得包含 signaling URL 或短期
        token。/ A server-authored defect must not be reported as a user-submitted 422; the error
        response never includes the signaling URL or ephemeral token.
    """

    try:
        validator.validate_definition("RealtimeConnection", payload)
    except DomainError as error:
        if error.problem.code != "contract.validation_failed":
            raise
        raise DomainError(
            Problem(
                "interview.realtime_connection_response_invalid",
                500,
                "Realtime connection configuration is invalid",
                retryable=True,
            )
        ) from None
